Title - Customer Engagement & Delivery Performance Dashboard
...
5) Evaluated customer acquisition channels (Ads, Organic, Returning customers)
6) Designed interactive slicers for state, product, return status, and year filtering.
7) Created KPI-driven visuals to summarize business performance.
8) Developed a clean, structured dashboard layout for executive decision-making.

Key Visuals:
1) KPI Cards: Summarize key business metrics such as total revenue, total units sold, returns, and satisfaction performance.
2) Line Chart (State-wise Revenue): Shows revenue distribution across states and identifies top-performing regions.
3) Bar Chart (Units Sold by State): Compares product demand across different states.
4) Column Chart (Returns by Year): Tracks return trends from 2017 to 2019.
5) Pie Chart (Product-wise Satisfaction): Displays satisfaction distribution across product categories.
6) Bar Chart (Customer Acquisition): Compares acquisition sources such as Ads, Organic, and Returning customers.
7) Pie Chart (Annual Sales Distribution): Shows proportional contribution of each state to total sales.

Filters/Slicers: Enable dynamic analysis by state, product, year, and return status
